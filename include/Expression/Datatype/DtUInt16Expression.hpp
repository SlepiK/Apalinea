...
namespace Apalinea::Expression::DataType {
    class [[maybe_unused]] DtUInt16Expression : public IExpression {
    public:
        static constexpr std::string_view IDENTIFIER = Core::Type::Datatype::DtUInt16::IDENTIFIER;

        [[maybe_unused]] explicit DtUInt16Expression(std::string&& entry) : IExpression({IDENTIFIER}), entry(std::move(entry)), load(true) {
        }

        [[maybe_unused]] explicit DtUInt16Expression(uint16_t&& value) : IExpression({IDENTIFIER}), load(false), data(value) {
        }
