...
namespace Apalinea::Expression::DataType {
    ...
    public:
        static constexpr std::string_view IDENTIFIER = Core::Types::Datatype::DtInt32::IDENTIFIER;

        explicit DtInt32Expression(std::string&& entry) : IExpression({IDENTIFIER}), entry(std::move(entry)), load(true) {
        }

        explicit DtInt32Expression(int32_t&& value) : IExpression({IDENTIFIER}), load(false), data(value) {
        }
        ...
        void execute() override {
            if(load) {
                ...
                if (this->tuple.containsItem(this->entry)) {
                    this->data = this->tuple.getEntry(this->entry).get<Core::Types::Datatype::DtInt32>();
                } else {
    ...
    private:
        ...
        std::string entry;
        Core::Types::Datatype::DtInt32 data;
        bool load;
    };
}
